...
class RestauranteDAO {
    ...
    public function insertRestaurante($restaurante) {
        $query = "INSERT INTO " . self::RESTAURANTE_TABLE . "( name, image, menu, minorprice, mayorprice) VALUES (?,?,?,?,?)";
        ...
        mysqli_stmt_bind_param($stmt, 'sssddi', $name, $image, $menu, $minorprice, $mayorprice);
        return $stmt->execute();
    }
    ...
    public function updateRestaurante($restaurante) {
        $query = "UPDATE " . self::RESTAURANTE_TABLE . " SET name=?, image=?, menu=?, minorprice=?, mayorprice=?,idCategory=? where id=?";
        $stmt = mysqli_prepare($this->conex, $query);
        $name = $restaurante->getName();
        $image = $restaurante->getImage();
        $menu = $restaurante->getMenu();
